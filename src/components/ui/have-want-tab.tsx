 ...
 * 계측(컴포넌트 시안 + EX-007): active = **secondary-900 글자** + primary-900 밑줄,
 * inactive = secondary-500 글자 + secondary-50 밑줄. 밑줄은 두 탭이 합쳐 화면 폭 전체.
    ...
    return (
      <button
        ...
        className={`flex-1 border-b-2 pb-2.5 text-center text-button1 ${
          active
            ? 'border-primary-900 text-secondary-900'
            : 'border-secondary-50 text-secondary-500'
        }`}
      >
        {label}
        {count != null && ` ${count}`}
